...
<head>
    ...
    <title>Member Honour Information</title>
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">查询成员荣誉信息</h1>
                <table width="100%" class="table table-striped table-bordered table-hover" id="myTable">
                    ...
                    <tbody>
                    <?php
                    $sql = "SELECT * FROM tbl_honour";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo '<tr><td>' . $row["Hono_ID"] . '</td><td>' . $row["Hono_Date"] . '</td><td>' . $row["Hono_Name"];
                            echo '</td><td>' . $reflection[$row["Hono_GrMe_ID"]] . '</td><td class="center">';
                            echo '<button type="button" class="btn btn-primary btnDelete" id="' . $row["Hono_ID"] . '">删除</button>';
                            echo '</td></tr>';
                        }
                    }
                    ?>
    ...
    $(".btnDelete").click(function () {
        ...
        if(confirm("确定删除？")) {
            $.ajax({
                ...
                type: "POST",
                data: {id: infoId, tableName: "honour"},
                dataType: "html",
                error: function() {

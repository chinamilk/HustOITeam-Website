...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO tbl_achievement VALUES(?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssss", $id, $date, $name, $identity, $type, $detailPath, $memberId);

    $id = createGuid();
    $date = checkInput($_POST["date"]);
    $name = checkInput($_POST["name"]);
    $identity = checkInput($_POST["identity"]);
    $type = checkInput($_POST["type"]);
    $detailPath = checkInput($_POST["detailPath"]);
...
<head>
    ...
    <title>Add Member Achievement Info</title>
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">添加成员科研成果、项目信息</h1>
                <form id="myForm" role="form" id="myForm" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="radio">
                            <label class="radio-inline">
                                <input type="radio" name="type" id="rTypePaper" value="论文" checked>论文
                            </label>
                            <label class="radio-inline">
                                <input type="radio" name="type" id="rTypeEnterprise" value="横向项目">横向项目
                            </label>
                            <label class="radio-inline">
                                <input type="radio" name="type" id="rTypeAcademic" value="纵向项目">纵向项目
                            </label>
                        </div>
                    ...
                    <div class="form-group">
                        <label>时间*</label>
                        <input class="form-control" name="date" id="date" placeholder="格式：论文(YYYY)/项目(YYYY.MM-YYYY.MM)">
                        <p class="help-block"></p>
                    </div>
                    <div class="form-group">
                        <label>名称*</label>
                    ...
                    <div class="form-group">
                        <label>参与身份</label>
                        <input class="form-control" name="identity" id="identity" readonly>
                        <p class="help-block"></p>
                    </div>
                    <div class="form-group">
                        <label>详细内容路径</label>
                        <input class="form-control" name="detailPath" id="detailPath" placeholder="从根路径开始">
                        <p class="help-block"></p>
                    </div>
                    <hr>

...
<head>
    ...
    <title>Member Achievement Information</title>
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">查询成员科研成果、项目信息</h1>
                <table width="100%" class="table table-striped table-bordered table-hover" id="myTable">
                    ...
                    <tbody>
                    <?php
                    $sql = "SELECT * FROM tbl_achievement ORDER BY Achi_Date DESC";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo '<tr><td>' . $row["Achi_ID"] . '</td><td>' . $row["Achi_Date"] . '</td><td>' . $row["Achi_Name"] . '</td><td>' . $row["Achi_Identity"];
                            echo '</td><td>' . $row["Achi_Type"] . '</td><td>' . $row["Achi_DetailPath"] . '</td><td>' . $reflection[$row["Achi_GrMe_ID"]] . '</td><td class="center">';
                            echo '<button type="button" class="btn btn-primary btnDelete" id="' . $row["Achi_ID"] . '">删除</button>';
                            echo '</td></tr>';
                        }
                    }
                    ?>
    ...
    $(".btnDelete").click(function () {
        ...
        if(confirm("确定删除？")) {
            $.ajax({
                ...
                type: "POST",
                data: {id: infoId, tableName: "achievement"},
                dataType: "html",
                error: function() {

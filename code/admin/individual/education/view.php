...
<head>
    ...
    <title>Member Education Information</title>
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">查询成员教育信息</h1>
                <table width="100%" class="table table-striped table-bordered table-hover" id="myTable">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo '<tr><td>' . $row["Educ_ID"] . '</td><td>' . $row["Educ_Date"] . '</td><td>' . $row["Educ_School"] . '</td><td>' . $row["Educ_Major"];
                            echo '</td><td>' . $row["Educ_Degree"] . '</td><td>' . $reflection[$row["Educ_GrMe_ID"]] . '</td><td class="center">';
                            echo '<button type="button" class="btn btn-primary btnDelete" id="' . $row["Educ_ID"] . '">删除</button>';
                            echo '</td></tr>';
                        }
                    }
                    ?>
    ...
    $(".btnDelete").click(function () {
        ...
        if(confirm("确定删除？")) {
            $.ajax({
                ...
                type: "POST",
                data: {id: infoId, tableName: "education"},
                dataType: "html",
                error: function() {

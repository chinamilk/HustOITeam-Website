...
<head>
    ...
    <title>Group News Information</title>
                ...
                <table width="100%" class="table table-striped table-bordered table-hover" id="myTable">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo '<tr><td>' . $row["News_ID"] . '</td><td>' . $row["News_Type"] . '</td><td>' . $row["News_Title"] . '</td><td>' . $row["News_ReleaseDate"] . '</td><td>' . $row["News_Date"] . '</td><td>' . $row["News_Publisher"] . '</td><td class="center">';
                            echo '<button type="button" class="btn btn-primary btnView" data-toggle="modal" data-target="#detail' . $row["News_ID"] . '">查看</button>';
                            echo '<button type="button" class="btn btn-primary btnDelete" id="' . $row["News_ID"] . '">删除</button>';

                            //detail panel
                            echo '<div class="modal fade" id="detail' . $row["News_ID"] . '" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">详细信息</h4></div>';
                            echo '<div class="modal-body">';
                            if($row["News_Path"] != "") {
                                echo '<p><img height="100%" width="100%" src="../../../' . $row["News_Path"] . '"></p>';
                            }
                            echo '<p class="h3">简要介绍</p>';
                            echo '<p align="left" style="text-indent: 25px;">' . $row["News_Introduction"] . '</p>';
                            echo '<p class="h3">详细介绍(以' . htmlspecialchars("<br>") . '作为换行判定)</p>';
                            echo '<p align="left" style="text-indent: 25px;">' . $row["News_Detail"] . '</p>';

...
<head>
    ...
    <title>Group Resource Information</title>
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">查询团队资源信息</h1>
                <table width="100%" class="table table-striped table-bordered table-hover" id="myTable">
                    ...
                    <tbody>
                    <?php
                    $sql = "SELECT * FROM tbl_resource";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo '<tr><td>' . $row["Reso_ID"] . '</td><td>' . $row["Reso_Type"] . '</td><td>' . $row["Reso_Name"] . '</td><td>' . $row["Reso_Date"] . '</td><td>' . $reflection[$row["Reso_GrMe_ID"]] . '</td><td class="center">';
                            echo '<button type="button" class="btn btn-primary btnView" data-toggle="modal" data-target="#detail' . $row["Reso_ID"] . '">查看</button>';
                            echo '<button type="button" class="btn btn-primary btnDelete" id="' . $row["Reso_ID"] . '">删除</button>';

                            //detail panel
                            echo '<div class="modal fade" id="detail' . $row["Reso_ID"] . '" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">详细信息</h4></div>';
                            echo '<div class="modal-body">';
                            echo '<p class="h3">下载密码</p>';
                            echo '<p>' . $row["Reso_Passward"] . '</p>';
                            echo '<p class="h3">下载次数</p>';
                            echo '<p>' . $row["Reso_Downlands"] . '</p>';
                            echo '<p class="h3">资源路径</p>';
                            echo '<p align="left" style="text-indent: 25px;">' . $row["Reso_Path"] . '</p>';
                            echo '<p class="h3">资源简介</p>';
                            echo '<p align="left" style="text-indent: 25px;">' . $row["Reso_Introduction"] . '</p>';
    ...
    $(".btnDelete").click(function () {
        ...
        if(confirm("确定删除？")) {
            $.ajax({
                ...
                type: "POST",
                data: {id: infoId, tableName: "resource"},
                dataType: "html",
                error: function() {

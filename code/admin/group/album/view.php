...
<head>
    ...
    <title>Group Album Information</title>
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">查询团队相册信息</h1>
                <table width="100%" class="table table-striped table-bordered table-hover" id="myTable">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo '<tr><td>' . $row["Albu_ID"] . '</td><td>' . $row["Albu_Title"] . '</td><td>' . $row["Albu_Date"] . '</td><td>' . $row["Albu_Introduction"] . '</td><td class="center">';
                            echo '<button type="button" class="btn btn-primary btnView" data-toggle="modal" data-target="#detail' . $row["Albu_ID"] . '">查看</button>';
                            echo '<button type="button" class="btn btn-primary btnDelete" id="' . $row["Albu_ID"] . '">删除</button>';

                            //detail panel
                            echo '<div class="modal fade" id="detail' . $row["Albu_ID"] . '" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">详细信息</h4></div>';
                            echo '<div class="modal-body">';
                            //table version
                            echo '<p><img height="100%" width="100%" src="../../../' . $row["Albu_Path"] . '"></p>';
        ...
        $(".btnDelete").click(function () {
            ...
            if(confirm("确定删除？")) {
                $.ajax({
                    ...
                    type: "POST",
                    data: {id: infoId, tableName: "album"},
                    dataType: "html",
                    error: function() {
